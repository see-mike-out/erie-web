...
export const noteScale: OctaveObject[] = [
  {
    c: 16.35,
    cs: 17.32,
    d: 18.35,
...
export function noteToFreq(note: number | string) {
  if (typeof note === 'number') return note;
  else if (typeof note === 'string') {
    let n = note[0]?.toLowerCase(), o = parseInt(note[1]), a = note[2]?.toLowerCase();
    if (a === "#") a = "s";
    else if (a === "♭") a = "b";
    if (o > 8) return null;
    if (a === "b") {
      let n_a = <keyof typeof sharpToFlat>(n + a);
